...
| ~~`episode-ros-ws/`~~ | ROS2 工作区 ⭐ **2026-09-04 已进 `SHOP-SELL/Episode1/过期暂存/`**，`$EPISODE_WS` 同日撤掉 | 停用（只做 VLA 三任务用不到；复活先搬回再 colcon build，路径别含中文） |
...
## 4. 上位机（从臂）前置要求（教程 3.4 §1.3，全人工；⭐ 2026-09-04 教程站重排，原 2.4）
...
| ~~`:8110`~~ | ROS 网页 teleop（2026-09-04 随 ROS 工作区停用） | — |
...
- 原教程第 4 章（Soma Zero 下棋，已摘出到 `AI与科创教学项目/`）走这两仓；第 3 章（VLA 实战，⭐ 2026-09-04 重排前为第 2 章）走本仓

## 7. 教程站章节地图

离线站 http://127.0.0.1:7788（`$EPISODE1_DOCS/教程/serve.sh` 起站）。
逐章进度与本仓命令对照 → [tutorial-progress.md](tutorial-progress.md)。
注意：教程第 3 章（重排前第 2 章）原文按「恩培魔改 fork」教学，**命令以本仓 `tasks/<slug>/TASK.md` 为准**（插件路线对照表见 tutorial-progress.md）。
